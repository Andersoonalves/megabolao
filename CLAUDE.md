...
- **Runtime**: Node.js 20+, TypeScript strict (`any` proibido)
- **Framework**: NestJS com versionamento URI (`/api/v1/`)
- **Auth**: Supabase Auth — JWT verificado localmente via `SUPABASE_JWT_SECRET`
- **DB**: Supabase Postgres — RLS ativo
- **Queue**: BullMQ + Redis
- **Integrações planejadas**: WhatsApp, Google Drive
- **Docs**: Swagger em `/docs` (fora de produção)
- **Tipos compartilhados**: `@nossobolao/shared-types` (`index.ts`)

## Papéis de usuário

| Papel | Acesso |
|-------|--------|
| `MASTER` | Todos os tenants; usa header `X-Tenant-Id` para escolher tenant |
| `ADMIN` | Apenas seu `tenant_id` do JWT; header ignorado |
| Participante (OTP) | `papel` ausente no JWT; RLS do Supabase governa |

## Regras críticas — nunca ignorar

1. **Toda query filtra por `tenant_id`** — sem exceção
2. **`SUPABASE_SERVICE_KEY` nunca vai ao frontend**
3. **TypeScript strict** — `any` proibido
4. **Toda rota tem `@Roles()`** — nenhum endpoint desprotegido acidentalmente
...
- Framework: Jest
- Padrão: **AAA** (Arrange / Act / Assert) com comentários `// Arrange`, `// Act`, `// Assert`
- Mocks do Supabase com `jest.fn()` no nível do módulo, `jest.clearAllMocks()` no `beforeEach`
- Nunca usar banco real nos testes unitários — apenas mocks do `SupabaseService`
...
```bash
supabase start          # Sobe Supabase local
supabase db reset       # Aplica schema + seed
npm run start:backend   # API em localhost:3000
npm run test:backend    # Testes unitários
npm run generate:types  # Regenera tipos OpenAPI em index.ts (abaixo de "// --- GERADO ---")
```
...
## Tipos compartilhados (`index.ts`)

Seção **manual**: ENUMs e interfaces de domínio (`Tenant`, `Bolao`, `Cota`, `Sorteio`, `Premio`, etc.).
Seção **gerada** (abaixo de `// --- GERADO ---`): não editar à mão — rodar `npm run generate:types`.

## Convenções de código

- ValidationPipe global: `whitelist: true`, `forbidNonWhitelisted: true`, `transform: true`
- CORS local: `origin: true`; produção: lista de domínios em `CORS_ORIGINS`
- Swagger só fora de produção
- Logger NestJS padrão em dev; substituir por Pino/Winston em produção
