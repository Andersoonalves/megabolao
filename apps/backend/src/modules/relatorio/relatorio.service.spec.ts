...
const makeBolao = () => ({ id: BOLAO_ID, tenantId: TENANT_ID, nome: 'Bolão Teste' });
...
describe('RelatorioService', () => {
  ...
  describe('gerarPdf', () => {
    it('gera PDF, faz upload com content-type correto e retorna URL', async () => {
      ...
      // Act
      const result = await service.gerarPdf(TENANT_ID, BOLAO_ID);

      // Assert
      expect(result.url).toBe('https://supabase.co/signed-url');
      expect(result.caminho).toContain('.pdf');
    });
    ...
    it('lança NotFoundException quando bolão não existe', async () => {
      ...
      // Act / Assert
      await expect(service.gerarPdf(TENANT_ID, BOLAO_ID)).rejects.toBeInstanceOf(NotFoundException);
    });
  });
});
