...
@Component({
  ...
  template: `
    ...
    <div class="bg-white border-b border-slate-200 px-4 lg:px-7 py-3 flex items-center justify-between gap-4 sticky top-14 lg:top-0 z-10">
      <span class="font-display font-semibold text-[14px] sm:text-base">{{ 'dashboardAdmin.title' | translate }}</span>
      <div class="flex gap-2">
        <a routerLink="/bolao/novo"
          ...
          + <span class="hidden sm:inline">{{ 'dashboardAdmin.newPool' | translate }}</span>
        </a>
      </div>
    </div>
    ...
    <div class="p-4 lg:p-7">
      ...
      <!-- Título -->
      <div class="mb-6">
        <h1 class="font-display text-[26px] font-semibold tracking-tight mb-1">{{ 'dashboardAdmin.overview' | translate }}</h1>
        <p class="text-slate-500 text-[13.5px]">{{ 'dashboardAdmin.subtitle' | translate }}</p>
      </div>
      ...
      @if (!loading()) {
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div class="bg-white border border-slate-200 rounded-lg p-[18px]">
            <div class="text-[11.5px] font-semibold text-slate-500 uppercase tracking-widest mb-1">{{ 'dashboardAdmin.kpiActivePools' | translate }}</div>
            <div class="font-display text-[28px] font-semibold tracking-tight tabular text-green-700">{{ emAndamento() }}</div>
            <div class="text-[11.5px] text-slate-400 mt-0.5">{{ 'dashboardAdmin.kpiTotal' | translate:{ n: bolaoes().length } }}</div>
          </div>

          <div class="bg-white border border-slate-200 rounded-lg p-[18px]">
            <div class="text-[11.5px] font-semibold text-slate-500 uppercase tracking-widest mb-1">{{ 'dashboardAdmin.kpiQuotas' | translate }}</div>
            <div class="font-display text-[28px] font-semibold tracking-tight tabular">{{ totalCotas() | number }}</div>
            <div class="text-[11.5px] text-slate-400 mt-0.5">{{ 'dashboardAdmin.kpiQuotasHint' | translate }}</div>
          </div>

          <div class="bg-white border border-slate-200 rounded-lg p-[18px]">
            <div class="text-[11.5px] font-semibold text-slate-500 uppercase tracking-widest mb-1">{{ 'dashboardAdmin.kpiRevenue' | translate }}</div>
            ...
            <div class="text-[11.5px] text-slate-400 mt-0.5">{{ 'dashboardAdmin.kpiRevenueHint' | translate }}</div>
          </div>

          <div class="bg-white border border-slate-200 rounded-lg p-[18px]">
            <div class="text-[11.5px] font-semibold text-slate-500 uppercase tracking-widest mb-1">{{ 'dashboardAdmin.kpiParticipants' | translate }}</div>
            <div class="font-display text-[28px] font-semibold tracking-tight tabular">{{ totalParticipantes() | number }}</div>
            <div class="text-[11.5px] text-slate-400 mt-0.5">{{ 'dashboardAdmin.kpiParticipantsHint' | translate }}</div>
          </div>
        </div>
        ...
        <div class="bg-white border border-slate-200 rounded-lg overflow-hidden">
          <div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <h2 class="font-display font-semibold text-[15px]">{{ 'dashboardAdmin.sectionPools' | translate }}</h2>
            <a routerLink="/boloes" class="text-xs text-green-700 font-semibold no-underline">{{ 'common.viewAll' | translate }}</a>
          </div>

          @if (bolaoes().length === 0) {
            <div class="p-12 text-center">
              <div class="text-4xl mb-3">🎲</div>
              <p class="text-slate-500 text-sm mb-4">{{ 'dashboardAdmin.emptyPools' | translate }}</p>
              <a routerLink="/bolao/novo" class="inline-flex items-center gap-1.5 px-4 py-2 bg-green-700 hover:bg-green-800 text-white text-sm font-semibold rounded-[10px] no-underline transition-colors">
                {{ 'dashboardAdmin.createFirst' | translate }}
              </a>
            </div>
          } @else {
                ...
                <div class="px-5 py-4 flex items-center gap-4 hover:bg-slate-50 transition-colors">
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-0.5">
                      ...
                      <span class="text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded-full flex-shrink-0"
                        ...
                        {{ ('dashboardAdmin.status.' + b.status) | translate }}
                      </span>
                    </div>
                    @if (b.dataInicio) {
                  ...
                  <div class="hidden sm:flex items-center gap-6 text-right flex-shrink-0">
                    <div>
                      ...
                      <div class="text-[10.5px] text-slate-400">{{ 'dashboardAdmin.cotasLabel' | translate }}</div>
                    </div>
                    <div>
                      <div class="font-semibold text-[13px] tabular text-green-700">{{ b.valorBrutoArrecadado | currency:'BRL':'symbol':'1.0-0' }}</div>
                      <div class="text-[10.5px] text-slate-400">{{ 'dashboardAdmin.raisedLabel' | translate }}</div>
                    </div>
                  </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
          <a routerLink="/participantes"
            ...
            <span class="text-[12px] font-semibold text-center group-hover:text-green-700">{{ 'dashboardAdmin.shortcutParticipants' | translate }}</span>
          </a>
          <a routerLink="/sorteios"
             class="bg-white border border-slate-200 rounded-xl p-4 flex flex-col items-center gap-2 hover:border-green-300 hover:bg-green-50 transition-colors no-underline text-slate-700 group">
            <span class="text-2xl">✦</span>
            <span class="text-[12px] font-semibold text-center group-hover:text-green-700">{{ 'dashboardAdmin.shortcutDraw' | translate }}</span>
          </a>
          <a routerLink="/whatsapp"
             class="bg-white border border-slate-200 rounded-xl p-4 flex flex-col items-center gap-2 hover:border-green-300 hover:bg-green-50 transition-colors no-underline text-slate-700 group">
            <span class="text-2xl">💬</span>
            <span class="text-[12px] font-semibold text-center group-hover:text-green-700">{{ 'dashboardAdmin.shortcutWhatsapp' | translate }}</span>
          </a>
          <a routerLink="/relatorios"
             class="bg-white border border-slate-200 rounded-xl p-4 flex flex-col items-center gap-2 hover:border-green-300 hover:bg-green-50 transition-colors no-underline text-slate-700 group">
            <span class="text-2xl">📄</span>
            <span class="text-[12px] font-semibold text-center group-hover:text-green-700">{{ 'dashboardAdmin.shortcutReports' | translate }}</span>
          </a>
        </div>
      }
    </div>
