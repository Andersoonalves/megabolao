        ...
        <div class="overflow-x-auto">
          <table class="w-full text-[13.5px]">
            <thead class="bg-slate-50">
              <tr>
                <th class="text-left text-[11px] font-semibold text-slate-400 uppercase tracking-widest px-5 py-3 min-w-[220px]">Empresa</th>
                ...
                <th class="text-left text-[11px] font-semibold text-slate-400 uppercase tracking-widest px-5 py-3 hidden lg:table-cell">Taxa</th>
        ...
        @if (!loading() && tenants().length > 0 && totalPages() > 1) {
          <div class="px-5 py-3 border-t border-slate-100 flex items-center justify-between">
            ...
            <div class="flex gap-1.5">
              <button (click)="prevPage()" [disabled]="page() <= 1" class="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-sm font-semibold disabled:opacity-40">Anterior</button>
              <button (click)="nextPage()" [disabled]="page() >= totalPages()" class="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-sm font-semibold disabled:opacity-40">Próxima</button>
            </div>
          </div>
        }
      </div>
    ...
    @if (editando()) {
      ...
      <div class="fixed right-0 top-0 h-full w-full sm:w-[440px] bg-white z-50 flex flex-col shadow-xl overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-slate-200 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 class="font-display font-semibold text-lg">Editar tenant</h2>
        ...
        <div class="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
          ...
          <!-- Slug -->
          <div>
            <label class="block text-xs font-semibold text-slate-500 mb-1.5 tracking-wide">Slug</label>
            <div class="flex items-center border border-slate-200 rounded-[10px] overflow-hidden focus-within:border-green-700 transition-colors">
              <span class="px-2.5 py-2.5 text-[11.5px] text-slate-400 bg-slate-50 border-r border-slate-200 whitespace-nowrap flex-shrink-0">nossobolao/</span>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-slate-500 mb-1.5 tracking-wide">Taxa adm. (%)</label>
          ...
          <!-- Cor primária -->
          <div>
            <label class="block text-xs font-semibold text-slate-500 mb-1.5 tracking-wide">Cor primária (branding)</label>
            ...
            <p class="text-[11px] text-slate-400 mt-1">Usado no avatar e no portal do participante</p>
          </div>

          <!-- Nome customizado -->
          <div>
            <label class="block text-xs font-semibold text-slate-500 mb-1.5 tracking-wide">Nome customizado (opcional)</label>
        ...
        <div class="px-6 py-4 border-t border-slate-200 flex gap-2.5 flex-shrink-0">
          <button (click)="fecharEdicao()" class="flex-1 py-2.5 bg-white border border-slate-200 font-semibold text-sm rounded-[10px] hover:bg-slate-50">Cancelar</button>
          <button (click)="salvarEdicao()" [disabled]="editLoading()"
...
export class TenantsComponent implements OnInit {
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      this.error.set(e.error?.message ?? `Erro ao carregar tenants${e.status ? ` [${e.status}]` : ''}. Exibindo dados de demonstração.`);
